...
impl Maps {
    ...
    #[inline]
    fn map_of(&self, ptr: *const u8) -> Option<&Map> {
        let ptr = ptr.cast_mut();

        self.maps.iter().find(|map| map.range.contains(&ptr))
    }

    #[inline]
    fn _assert_permissions(&self, ptr: *const u8, permissions: Permissions) -> bool {
        self.map_of(ptr)
            .map(|map| map.permissions().contains(permissions))
            .unwrap_or_default()
    }
    ...
    pub(crate) fn assert_permissions(ptr: *const u8, permissions: Permissions) -> bool {
        Self::current()
            .map(|maps| maps._assert_permissions(ptr, permissions))
            .unwrap_or_default()
    }
}
